...
class CalendarViewModelTest {
    ...
    private val oliveRepository = mockk<OliveRepository>()
    ...
    fun `값을_입력하면_데이터_추가_후_다이얼로그_끄기`() {
        runTest {
            // given
            val viewModel = CalendarViewModel(cashBookRepository, oliveRepository)
            val time = "2022-12-15"
            val name = "버거킹"
            val amount = "-50000"
            coEvery { cashBookRepository.getAllUserData() } returns BaseResponse.Success(emptyList())
